...
from bridge_dataset import BridgeDataset, glob_to_path_list
...
def parse_args():
    ...
    parser.add_argument(
        "--per_worker_batch_size", type=int, default=1, help="Batch size per worker"
    )
    parser.add_argument(
        "--max_train_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help=("log training metric every X steps to `--report_to`"),
    )
    parser.add_argument(
        "--report_to",
        type=str,
...
def get_dataset(args, tokenizer):
    train_paths = [glob_to_path_list(["*.tfrecord*"], prefix=args.train_data_dir, exclude=[])]
    dataset = BridgeDataset(
        train_paths,
        args.seed + jax.process_index(),
...
def main():
    ...
    # Get the datasets: you can either provide your own training and evaluation files (see below)
    train_dataloader = get_dataset(args, tokenizer)
    ...
    logger.info(f"  Num training iters = {args.max_train_steps}")
    logger.info(f"  Total train batch size (w. parallel & distributed) = {args.per_worker_batch_size // jax.local_device_count() * jax.device_count()}")
    logger.info(f"  Batch size per device = {args.per_worker_batch_size // jax.local_device_count()}")
        ...
        wandb.config.update(
            {
                ...
                "total_train_batch_size": args.per_worker_batch_size // jax.local_device_count() * jax.device_count(),
                "total_optimization_step": args.max_train_steps,
